...
export default function MagnetLetter({ letter, color }: MagnetLetterProps) {
  ...
  const shadowColor = color.replace('#', '').replace(/^(.{2})(.{2})(.{2})$/, (match, r, g, b) => {
  ...
  const lighterShadow = color.replace('#', '').replace(/^(.{2})(.{2})(.{2})$/, (match, r, g, b) => {
  ...
  const evenDarker = color.replace('#', '').replace(/^(.{2})(.{2})(.{2})$/, (match, r, g, b) => {
    const darken = (hex: string) =>
  ...
  const darkest = color.replace('#', '').replace(/^(.{2})(.{2})(.{2})$/, (match, r, g, b) => {
    const darken = (hex: string) =>
